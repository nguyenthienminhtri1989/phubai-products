...
async function main() {
  // 1. Check page_registry
  const pageCount = await prisma.pageRegistry.count();
  console.log(`\n📄 page_registry: ${pageCount} trang`);
  ...
  // 3. If page_registry is empty, seed it
  if (pageCount === 0) {
    console.log(`\n⚠️  page_registry trống! Đang seed lại...`);
    const pages = [
    ...
    console.log(`✅ Đã seed ${pages.length} trang vào page_registry`);
  } else {
    console.log(`\n✅ page_registry OK`);
  }
}
